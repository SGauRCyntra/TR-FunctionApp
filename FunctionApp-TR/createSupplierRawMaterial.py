...
class createSupplierRawMaterial():
    ...
    # Create client
        try:
            ...
            headers = ["ParentFormID", "RawMaterialID", "RawMaterialName", "Status", "SupplierNo", "RawMaterialNo"]
            headers_to_change = [" ParentFormID", "AssurX_No", "LongDescription" ,"QA_Status" ,"Vendor_No" ,"Item_No"]
            orderAdd = []
            for i in range(len(self._payload)):
                dic ={}
                for j in range(len(headers)):
                    dic[headers_to_change[j]] = self._payload[i][headers[j]]
                if dic["QA_Status"] == "Active":
                    dic["QA_Status"] = "APPROVED"
                else:
                    dic["QA_Status"] = "DISQUALIFIED"
